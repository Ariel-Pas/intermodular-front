...
export class EmpresasApiService extends IEmpresasService{
  ...
  getEmpresas(): Observable<IEmpresaDisplay[]> {
    return this.httpClient.get<EmpresaJson[]>('http://localhost:3000/companies').pipe(
  ...
  getEmpresa(idEmpresa : string): Observable<IEmpresaDisplay> {
    return this.httpClient.get<EmpresaJson>(`http://localhost:3000/companies/${idEmpresa}`).pipe(
      map((empresa => {
